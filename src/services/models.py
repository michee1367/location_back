...
def model_class_to_model_info(model_class, jsonDataLegend) :
        if not (hasattr(model_class, "__fillables__")) :
            return None
        ...
        info = {
            ...
            "enums":get_enums_from_model(model_class)
        }
        """if jsonDataLegend and jsonDataLegend[getattr(model_class, "__name__")] :
            info["legend"]=jsonDataLegend[getattr(model_class, "__name__")]"""
            
        
        if hasattr(model_class, PROP_NAME_MIDDLEWARE_ACTION):
            middlware_actions = middleware_list_to_dict(getattr(model_class, PROP_NAME_MIDDLEWARE_ACTION))
            info["middlware_actions"] = middlware_actions
            #print(getattr(model_class, PROP_NAME_MIDDLEWARE_ACTION))
        
        return info
    
def get_models_by_super_class(super_class, surper_model_info, jsonDataLegend) :
    models_info = {}
    if not issubclass(super_class, db.Model) :
        return models_info
    for model_class in super_class.__subclasses__():
        model_info = model_class_to_model_info(model_class, jsonDataLegend)
        
        if not model_info :
            continue
        
        if surper_model_info :
        
            model_info["fields"] = {**surper_model_info["fields"], **model_info["fields"]}
            model_info["relations"] =  {**surper_model_info["relations"], **model_info["relations"]}
            model_info["fields_relations"] = {**surper_model_info["fields_relations"], **model_info["fields_relations"]}
...
def get_models(): 
    ...
    file_path_legend = "data/schema_convert.json"
    
    with open(file_path_legend) as f:
        jsonDataLegend = json.load(f)
    ...
    for model_class in db.Model.__subclasses__():
        ...
        if not (len(model_class.__subclasses__()) == 0) :
            ...
            sub_models_info = get_models_by_super_class(model_class, model_info, jsonDataLegend) 
...
def get_model_classes_from_super_class(super_class):
    return [cls for cls in super_class.__subclasses__() ]
...
def get_model_classes():
    ...
    for model_class in db.Model.__subclasses__():
        if not (len(model_class.__subclasses__()) == 0) :
            model_classes_super_class = get_model_classes_from_super_class(model_class)
            model_classes = [*model_classes, *model_classes_super_class]
        model_classes.append(model_class)
        
    return model_classes
def get_model_class_by_tablename(table_name) :
    return next((cls for cls in get_model_classes() if cls.__tablename__ == table_name), None)
...
def record_to_dict(record, widthRel=False):
    ...
    print(record.__mapper__.columns)
