...
class UserChildable(Middleware) :
    
    def __init__(self, clsChild : type, wording: str):
        self.clsChild = clsChild
    ...
    def handle(self, entity : User) :
        if not (type(entity) is User):
            return jsonify({"message":"entity n'est un utilisateur libre"})
        entity
        phone = entity.phone
    ...
    def check(*args, **kwargs) :
        ...
        clsChild = kwargs.get("clsChild")
        wording = kwargs.get("wording")
        
        return issubclass(clsChild, User) and wording
